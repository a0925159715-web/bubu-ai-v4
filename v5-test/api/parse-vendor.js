module.exports = async function handler(req,res){
 ...
 try{
  const apiKey=process.env.OPENAI_API_KEY;if(!apiKey)return res.status(500).json({error:"OPENAI_API_KEY is not set"});
  const {rawText,vendorCode,selectedCategory,mode}=req.body||{};
  const system=`你是台灣網拍賣家的廠商原文解析AI。只解析商品資料與生成文案，價格由網站計算。請回傳純 JSON：{"productName":"","colors":"","specs":"","sizeText":"","capacity":"","cost":0,"category":"clothing","copy":"","labelPurpose":"","labelUsage":"","labelStorage":"","labelNotice":"","labelOrigin":"韓國"}
...
分類：衣服褲裙洋裝 clothing；保養彩妝 skincare；清潔居家生活用品 life。
保養品：不需要顏色/服飾尺寸。capacity 抓容量/規格如 100ml、30ml、10入。生成中文標籤欄位 labelPurpose,labelUsage,labelStorage,labelNotice,labelOrigin。
...
文案繁體中文，台灣網拍闆娘口吻，不要中國用語，不要提真實廠商名。copy 格式：文案：...\\n\\n提醒：...\\n\\nHashtag：...`;
  ...
  const result=await response.json();if(!response.ok)return res.status(response.status).json({error:result.error?.message||"OpenAI API error"});
